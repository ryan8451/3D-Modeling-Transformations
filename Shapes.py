...
class Head(Shape):
    def __init__(self, position, shaderProg, size, color=ColorType.YELLOW):
        # Extract vertex and index data specifically for the head
        # Assuming getVertexData is a function that extracts data from the .dae file
        vertices, indices = getVertexData("assets/Wingman.dae", id="AB_AggroBot_S0_Q_Spikebot_Skelmesh_LOD0_ao_001_Head")

        # Call the parent class constructor to set up the shape
        super(Head, self).__init__(position, shaderProg, size, vertices, indices, color)
